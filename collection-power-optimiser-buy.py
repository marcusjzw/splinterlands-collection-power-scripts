...
saleDetailsRaw = requests.get("https://game-api.splinterlands.com/market/for_sale_grouped")
saleDetails = saleDetailsRaw.json()
...
idLowestPriceDict = {}
for i in range(len(saleDetails)):
    idLowestPriceDict[i] = (idNameDict[int(saleDetails[i]['card_detail_id'])][0], saleDetails[i]['card_detail_id'], saleDetails[i]['gold'], saleDetails[i]['edition'], idNameDict[int(saleDetails[i]['card_detail_id'])][1], saleDetails[i]['low_price'], saleDetails[i]['low_price_bcx'])
...
for i in range(len(idLowestPriceDict)):
    ...
    collectionPowerDict[i] = {'Name': idLowestPriceDict[i][0], 'isGold': idLowestPriceDict[i][2], 'CollectionPower: ': collectionPower, 'LowestPriceInUSD': idLowestPriceDict[i][5], 'CollectionPowerPerUSD': round(collectionPower / idLowestPriceDict[i][5], 2)}

# print(collectionPowerDict)

# we want to sort by collection power attained per DEC and find the best deals at the top

collectionPowerDict = [(k, collectionPowerDict[k]) for k in sorted(collectionPowerDict, key=lambda x: collectionPowerDict[x]['CollectionPowerPerUSD'], reverse=False)]
